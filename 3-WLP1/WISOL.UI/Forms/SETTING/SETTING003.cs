    ...
    public partial class SETTING003 : PageType
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(txtChemicalName.EditValue.NullString()))
                {
                ...
                if (string.IsNullOrEmpty(rdgLocation.EditValue.NullString()))
                {
                    MsgBox.Show("HÃY CHỌN VỊ TRÍ CỦA KHO HÀNG", MsgType.Warning);
                    return;
                }
                ...
                base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_SETTING003.PUT_ITEM"
                    ...
                    , new string[] {
                        ...
                        txtChemicalName.EditValue.NullString().Trim(),
                        gleUnit.EditValue.NullString(),
                        txtQuantitative.EditValue.NullString().ToUpper().Trim(),
                        txtExpiryHour.EditValue.NullString(),
                        gleMaker.EditValue.NullString(),
                        b64,
                        gleStageUse.EditValue.NullString(),
                        txtMinStock.Text.Trim(),
                        txtLeadTime.EditValue.NullString(),
                        txtConsume.EditValue.NullString(),
                        txtReplacementPeriod.EditValue.NullString(),
                        txtPriceUSD.EditValue.NullString(),
                        Consts.USER_INFO.Id,
                        rdgLocation.EditValue.NullString()
                    }
                    );
        ...
        private void gvList_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            try
            {
                ...
                else
                {
                    try
                    {
                        base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_SETTING003.GET_ITEM_DETAIL"
                            , new string[] { "A_CODE"
                            }
                            , new string[] { gvList.GetDataRow(e.RowHandle)["CODE"].NullString()
                            }
                            );
                        if (base.m_ResultDB.ReturnInt == 0)
                        {
                            ...
                            txtChemicalName.EditValue = dt.Rows[0]["CHEMICAL_NAME"].NullString();
                            ...
                            string loc = dt.Rows[0]["LOCATION"].NullString();

                            if(loc == "INSIDE")
                            {
                                rdgLocation.SelectedIndex = 0;
                            }
                            if(loc == "OUTSIDE")
                            {
                                rdgLocation.SelectedIndex = 1;
            ...
            if (dialogResult == DialogResult.OK)
            {
                try
                {
                    base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_SETTING003.DELETE_ITEM"
                        , new string[] { "A_CODE", "A_TRAN_USER_ID"
                        }
