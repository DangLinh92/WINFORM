...
{
    public partial class REPORT018 : PageType
    {
        public REPORT018()
        {
            InitializeComponent();
        ...
        private void C1()
        {
            if (txtFromWeek.EditValue is null)
            {
                MsgBox.Show("Hãy chọn tuần", MsgType.Warning);
                return;
            }
            ...
            DataTable results = CreateChartData(intYear, intWeekFrom, cbShiftWork.EditValue.ToString());
        ...
        private DataTable CreateChartData(int intYear, int intWeek, string shift)
        {
            ...
            {
                firstDayOfWeek = FirstDateOfWeekISO8601(intYear, intWeek);
                count = 7;
            }

            string end_day = firstDayOfWeek.AddDays(count).ToString("yyyy-MM-dd");
            try
            {
                base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT018.GET_LIST"
                , new string[] { "A_WEEK", "A_FIRST_DAY", "A_END_DAY", "A_SHIFT_WORK"
                }
                , new string[] {week, 
