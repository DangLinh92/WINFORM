    ...
    public partial class REPORT002 : PageType
    {
            ...
            try
            {
                if (Consts.DEPARTMENT == "ESD")
                {
                    ...
                    if (base.m_ResultDB.ReturnInt == 0)
                    {
                        ...
                        int warning = Convert.ToInt32(base.m_ResultDB.ReturnDataSet.Tables[1].Rows[0][3].ToString());
                        int warning2 = Convert.ToInt32(base.m_ResultDB.ReturnDataSet.Tables[1].Rows[0][4].ToString());
                        lblReport.Text = "AMOUNT: " + (c + u) + " / CHECKED: " + c + " / UNCHECKED: " + u + " / NG: " + ng + " / WARNING_2: " + warning + " / WARNING_1: " + warning2;
                ...
                if (Consts.DEPARTMENT == "EOS")
                {
                    base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT002.GET_LIST_EOS"
                    , new string[] { "A_PLANT", "A_DEPARTMENT", "A_TRAN_USER", "A_LANG",
                                 "A_DATE", "A_NEXT_DATE", "A_SHIFT_WORK", "A_STATUS", "A_SHOW_NG"},
                      new string[] { Consts.PLANT, Consts.DEPARTMENT, Consts.USER_INFO.Id, Consts.USER_INFO.Language,
                                 date, next_date, shift, status, showNG}
                    ...
                    if (base.m_ResultDB.ReturnInt == 0)
                    {
                        ...
                        int u = Convert.ToInt32(base.m_ResultDB.ReturnDataSet.Tables[1].Rows[0][1].ToString());
                        int ng = Convert.ToInt32(base.m_ResultDB.ReturnDataSet.Tables[1].Rows[0][2].ToString());
                        lblReport.Text = "AMOUNT: " + (c + u) + " / CHECKED: " + c + " / UNCHECKED: " + u + " / NG: " + ng;
                        lblReport.Font = new Font("Tahoma", 8.75f, FontStyle.Bold);
        ...
        private void gvList_RowClick(object sender, RowClickEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                ...
                try
                {
                    base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT002.GET_LIST_DETAIL"
                    , new string[] { "A_PLANT", "A_DEPARTMENT", "A_TRAN_USER", "A_LANG",
                    ...
                    if (base.m_ResultDB.ReturnInt == 0)
                    {
                        ...
                        c_code = base.m_ResultDB.ReturnDataSet.Tables[0].Rows[0]["DEVICE_CODE"].ToString();
                        c_name = base.m_ResultDB.ReturnDataSet.Tables[0].Rows[0]["DEVICE_NAME"].ToString();
                        c_item_name = base.m_ResultDB.ReturnDataSet.Tables[0].Rows[0]["ITEM_CHECK"].ToString();
            ...
            if (e.RowHandle >= 0)
            {
                ...
                //{
                    if ((!string.IsNullOrWhiteSpace(min_val)) && (!string.IsNullOrWhiteSpace(max_val)))
                    {
                        ...
                        POP.POP_REPORT002_CHART popup = new POP.POP_REPORT002_CHART(c_item_id, c_code, c_name, c_item_name, previous_date, current_date);
                        popup.ShowDialog();
                    }
                //}
