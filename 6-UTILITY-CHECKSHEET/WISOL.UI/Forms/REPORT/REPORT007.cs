...
namespace Wisol.MES.Forms.REPORT
{
    public partial class REPORT007 : PageType
    {
        ...
        public override void InitializePage()
        {
            ...
            try
            {
                base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT007.INT_LIST"
        ...
        public override void SearchPage()
        {
            ...
            string month = dtpMonth.DateTime.ToString("yyyy-MM") + "-01";
            string next_month = dtpMonth.DateTime.AddMonths(1).ToString("yyyy-MM") + "-01";
            string date = month; //dtpDate.DateTime.ToString("yyyy-MM-dd");
            string next_date = next_month;  //dtpDate.DateTime.AddDays(1).ToString("yyyy-MM-dd");
            g_date = month;
            g_next_date = next_month;
            c_code = string.Empty;
            c_name = string.Empty;
            ...
            try
            {
                ...
                if (base.m_ResultDB.ReturnInt == 0) 
                {
                    if(showNG == "FALSE")
                    {
                        ...
                        gvList.Columns["DEVICE_CODE"].Visible = false;
                    }
                    else
                    {
                        //g_date = dtpDate.DateTime.ToString("yyyy-MM-dd");
        ...
        private void gvList_RowClick(object sender, RowClickEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                ...
                try
                {
                    base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT007.GET_LIST_DETAIL"
                    , new string[] { "A_PLANT", "A_DEPARTMENT", "A_TRAN_USER", "A_LANG",
        ...
        private void imageSlider1_Click(object sender, EventArgs e)
        {
            ...
            {
                POP.POP_REPORT007 popup = new POP.POP_REPORT007(dt_url);
                popup.ShowDialog();
            }
        }
        ...
        private void gvList2_RowClick(object sender, RowClickEventArgs e)
        {
            if(e.RowHandle >= 0)
            {
                ...
                else
                {
                    if ((!string.IsNullOrWhiteSpace(min_val)) && (!string.IsNullOrWhiteSpace(max_val)))
                    {
                        string previous_date = DateTime.Today.AddMonths(-1).ToString("MM-dd").ToString();
                        string current_date = DateTime.Today.ToString("MM-dd").ToString();
