...
{
    public partial class REPORT023 : PageType
    {
        ...
        public override void InitializePage()
        {
            ...
            try
            {
                base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT023.INT_LIST"
        ...
        public override void SearchPage()
        {
            ...
            string ChuaSuDung = string.Empty;
            string ShowAlertItem = string.Empty;
            ...
            try
            {
                base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT023.GET_LIST"
                , new string[] { "A_PLANT", "A_DEPARTMENT", "A_TRAN_USER", "A_LANG",
                                 "A_DATE", "A_NEXT_DATE", "A_SHIFT_WORK", "A_STATUS", "A_SHOW_NG", "A_MACHINESTOP","A_SHOW_ALERT"},
                  new string[] { Consts.PLANT, "", Consts.USER_INFO.Id, Consts.USER_INFO.Language,
                                 date, next_date, shift, status, showNG, machineStop,ShowAlertItem}
                );
                if (base.m_ResultDB.ReturnInt == 0)
                {
                    if (showNG == "FALSE")
                    {
                        ...
                        if (ShowAlertItem == "TRUE")
                        {
                            base.m_BindData.BindGridView(gcList2, base.m_ResultDB.ReturnDataSet.Tables[1]);
                        }
                    }
                    else
                    {
                    ...
                    DataTable tableResult = ShowAlertItem == "TRUE" ? base.m_ResultDB.ReturnDataSet.Tables[2] : base.m_ResultDB.ReturnDataSet.Tables[1];
                    int c = Convert.ToInt32(tableResult.Rows[0][0].ToString());
                    int u = Convert.ToInt32(tableResult.Rows[0][1].ToString());
                    int ng = Convert.ToInt32(tableResult.Rows[0][2].ToString());
                    int alert = Convert.ToInt32(tableResult.Rows[0][3].ToString());
                    lblReport.Text = "AMOUNT:" + (c + u) + " / CHECKED:" + c + " / UNCHECKED:" + u + " / ALL NG ITEM:" + ng + " / ALERT:"+ alert;
        ...
        private void gvList_RowClick(object sender, RowClickEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                ...
                try
                {
                    base.m_ResultDB = base.m_DBaccess.ExcuteProc("PKG_REPORT023.GET_LIST_DETAIL"
                    , new string[] { "A_PLANT", "A_DEPARTMENT", "A_TRAN_USER", "A_LANG",
        ...
        private void cheAlert_CheckedChanged(object sender, EventArgs e)
        {
            chkNG.Checked = false;
            chkMachineStop.Checked = false;
        }
    }
}
